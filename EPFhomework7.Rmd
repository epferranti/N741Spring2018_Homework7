...
fitall <- rpart::rpart(mcs ~ age + female + pss_fr + 
                              homeless + pcs + cesd, 
                              data = h1a)
rpart::printcp(fitall) 
rpart::plotcp(fitall) 
summary(fitall) 
plot(fitall, uniform = TRUE, compress = FALSE, main = "Regression Tree for MCS Scores from Help Data")
text(fitall, use.n = TRUE, all = TRUE, cex = 0.5)
...
summary(fitk)
plot(fitk, uniform = TRUE, 
     main = "Classification Tree for MCS")
text(fitk, use.n = TRUE, all = TRUE, cex = 0.8)
...
class(fitallpk)
plot(fitallpk, main = "Conditional Inference Tree for MCS")
...
```{r}
library(rpart)
whoIsDepressed2 <- rpart::rpart(mcs_lt45 ~ age + female + 
...
library(partykit)
# Plot the tree
plot(partykit::as.party(whoIsDepressed2))
...
# Select the variables
varsel_msc <- randomForestSRC::var.select(fitallrf2)
glimpse(varsel_msc)

# Save the gg_minimal_depth object for later use
gg_md2 <- ggRandomForests::gg_minimal_depth(varsel_msc)
# Plot the object
plot(gg_md2)
...
plot(gg_v2, xvar2 = xvar2, panel = TRUE, alpha = 0.4) +
  labs(y="Predicted MSC reading", x="")

```
Link to HW: [my github link](https://github.com/epferranti/N741Spring2018_Homework7)
